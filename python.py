...
class Handler(BaseHandler):
    ...
    def do_POST(self):
        putanja = self.path  #  koju je putanju poslao js
        metod = self.command  #  koja je motoda koriscena

        if putanja == '/novi-zadatak':
            # duzina sadrzaja
            duzina_sadrzaja = int(self.headers['Content-Length'])
            # duzina sadrzaja pretvaramo u podatke citljive coveku
            ...
            # json.dumps(odgovor) pretvara listu odgovor u string i takavog ga salje korisniku u js
        ...
        if putanja == '/brisanje':
            ...
            duzina_sadrzaja = int(self.headers['Content-Length'])
            sadrzaj = self.rfile.read(duzina_sadrzaja).decode("utf-8")
            # kroz petlju trazimo taj zadatak i brisemo ga iz liste odgovor
